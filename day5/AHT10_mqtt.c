...
static int example_publish(void *handle, float humi, float temp)
{
    int res = 0;
    const char *fmt = "/%s/%s/user/get";
    char *topic = NULL;
    int topic_len = 0;
    char payload[128];  // 预留足够空间给 payload

    // 构建 JSON payload
    snprintf(payload, sizeof(payload), "{\"humi\": %.2f, \"temp\": %.2f}", humi, temp);
...
static int mqtt_example_main(void* params)
{
    void                   *pclient = NULL;
    int                     res = 0;
    int                     loop_cnt = 0;
   ...
   while (1) {
        ...
        // 每20次循环发送一次数据
        if (0 == loop_cnt % 20) {
            example_publish(pclient, humi, temp);
        }

        IOT_MQTT_Yield(pclient, 200);//等待调用时间
        rt_thread_mdelay(1000/20);//循环20次，只是为了缩短时间，时间不准
...
{
    // 创建线程
    AHT10 = rt_thread_create("AHT10", mqtt_example_main, RT_NULL, THREAD_STACK_SIZE, THREAD_PRIORITY, THREAD_TIMESLICE);
    // 创建成功就启动
    if (AHT10 != RT_NULL)
    {
